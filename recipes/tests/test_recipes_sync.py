...
def test_sync_recipe(
    ...
    result = runner.invoke(sync_recipes.sync_recipes, args=["--mode", "export", "--all", yml_export_file])
    assert result.exit_code == 2
    assert "Export mode should be used" in result.output

    result = runner.invoke(sync_recipes.sync_recipes,
                           args=["--mode", "export", "--public-recipes", yml_export_file])
